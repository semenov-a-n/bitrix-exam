    ...
    <div class="adm-nav-pages-block">
            ...
            if ($arResult["nStartPage"] <= 2 || $arResult["NavPageCount"] - $arResult["nStartPage"] <= 1 || abs($arResult['nStartPage'] - $arResult["NavPageNomer"]) <= 2) {
                ...
                if ($bPoints) {
                    ...
                    if ($curEndPage - $NavRecordGroupPrint > 1) {
                        ...
                        ?><a class="adm-nav-page adm-nav-page-separator"
                             href="<?= $arResult["sUrlPath"] ?>?<?= $strNavQueryString ?>PAGEN_<?= $arResult["NavNum"] ?>=<?= $hrefPageNumber ?>"
                             pagenum="<?= $hrefPageNumber ?>"></a><?
                    } else {
        ...
        <div class="adm-nav-pages-block">
            ...
            <? if ($arResult["bShowAll"] && !$arResult["NavShowAll"]): ?>
                ...
                <? else: ?>
                    <a class="adm-nav-page adm-nav-page-prev" href="<?= $sPrevHref; ?>"
                       id="<?= $ClientID ?>_previous_page"></a>
                <? endif; ?>
            <? endif; ?>
            <?
            $bFirst = true;
                ...
                if ($arResult["nStartPage"] <= 2 || $arResult["nEndPage"] - $arResult["nStartPage"] <= 1 || abs($arResult['nStartPage'] - $arResult["NavPageNomer"]) <= 2) {
                    ...
                    if ($bPoints) {
                        ...
                        if ($curEndPage - $arResult['nStartPage'] > 1) {
                            ...
                            ?><span class="adm-nav-page adm-nav-page-separator disabled"
            ...
            if ($arResult["bShowAll"]):
                if ($arResult["NavShowAll"]):
                    ?>
                    <a class="adm-nav-page"
                       href="<?= $arResult["sUrlPath"] ?>?<?= $strNavQueryString ?>SHOWALL_<?= $arResult["NavNum"] ?>=0"><?= GetMessage("navigation_paged") ?></a>
                    <?
                else:
                    ?>
                    <!-- next -->
                ...
                <? else: ?>
                    <a class="adm-nav-page adm-nav-page-next" href="<?= $sNextHref; ?>"
                       id="<?= $ClientID ?>_next_page"></a>
                <? endif; ?>

                    <a class="adm-nav-page"
                       href="<?= $arResult["sUrlPath"] ?>?<?= $strNavQueryString ?>SHOWALL_<?= $arResult["NavNum"] ?>=1"><?= GetMessage("navigation_all") ?></a>
                    <?
                endif;
            endif;
            ?>
